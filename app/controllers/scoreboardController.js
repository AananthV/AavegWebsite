...
// API
const apiScoreboard = async (req, res) => {
  try {
    const scoreData = await Score.aggregate([
      {
        $lookup: {
...
const apiEventScore = async (req, res) => {
  try {
    returnData = await getEventScores(req.params.eventId)
    return res.send(returnData)
  } catch (err) {
    logger.error(err)
    return res.status(500).send({ title: 'Error', error: 'Internal server error' })
  }
}
